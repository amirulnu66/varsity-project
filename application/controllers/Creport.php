...
// supplier stock report supplier id wise
	public function stock_report_supplier_id_wise(){
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Creport/stock_report_supplier_id_wise/');
        $config["total_rows"] = $this->Reports->product_counter_by_supplier($supplier_id);	
        $config["per_page"] = 10;
        $config["uri_segment"] = 3;
        $config["num_links"] = 5; 
        ...
        #pagination ends
        #  
        $content =$this->lreport->stock_report_supplier_id_wise($supplier_id,$date,$links,$config["per_page"],$page);
		$this->template->full_admin_html_view($content);
	}
	//Stock report supplir report
